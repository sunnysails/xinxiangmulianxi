        ...
        <section class="content">
            ...
            <div class="box box-solid box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">新增外包流水</h3>
                </div>
                <div class="box-body">
                    <%--公司 Lease--%>
                    <div class="box">
                        <div class="box-header with-border">
                            <h3 class="box-title">新外包公司信息</h3>
                        </div>
                        <div class="box-body">
                            <div class="row">
                                <div class="col-md-4">
                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <table class="table">
                                ...
                                <tr v-for="worker in deviceArray">
                                    <td>{{worker.name}}</td>
                                    <td>{{worker.unit}}</td>
                                    <td>{{worker.price}}</td>
                                    <td>{{worker.num}}</td>
                                    <td>{{worker.total}}</td>
                                    <td><a href="javascript:;" @click="remove(worker)"><i
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="">
                        ...
                        <div class="form-group">
                            <label>外包单价</label>
                            <input type="text" class="form-control" id="outPrice" readonly>
                        </div>
                        <div class="form-group">
                            <label>外包数量</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" @click="addWorker">加入列表</button>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    </div>
...
<script>
    ...
    $(function () {
        ...
        $("#workerId").change(function () {
            var id = $(this).val();
            if (id) {
                $.get("/business/out/worker.json", {'id': id}).done(function (resp) {
                    if (resp.status == 'success') {
                        var worker = resp.data;
                        $("#workerName").val(worker.workerViewName);
                        $("#currNum").val(worker.workerNowNum);
                        $("#unit").val(worker.workerUnit);
                        $("#outPrice").val(worker.workerUnitPrice);
                    } else {
                        alert(resp.message);
                    }
                }).error(function () {
        ...
        methods: {
            addWorker: function () {
                ...
                if (!flag) {
                    var json = {};
                    json.id = id;
                    json.name = $("#workerName").val();
                    json.unit = $("#unit").val();
                    json.price = $("#outPrice").val();
                    json.num = $("#outNum").val();
            ...
            remove: function (worker) {
                layer.confirm("确定要删除吗", function (index) {
                    app.$data.deviceArray.splice(app.$data.workerArray.indexOf(worker), 1);
                    layer.close(index);
                });
            },
            saveRent: function () {
                    ...
                    success: function (data) {
                        if (data.status == 'success') {
                            layer.confirm("保存成功", {btn: ['继续添加', '打印合同']}, function () {
                                ...
                                window.location.href="/business/out/"+data.data;
                            });
                        }
                    },
                    error: function () {
